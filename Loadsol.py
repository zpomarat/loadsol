...
class Loadsol:
    ...
    def convert_txt_to_csv(self, output_path:str):
        ...
        with open(output_path + output_file, "w", newline="", encoding="utf-8") as outfile:
    ...
    def get_time(self):
        """Returns an array containing the time data."""
        raw_data = self.csv_reader_loadsol()
        self.time = raw_data[:,0]
        return self.time
    
    def get_left_force(self):
        """Returns an array containing the force data of the left insole (Fheel, Fmedial, Flateral, Ftotal)."""
        raw_data = self.csv_reader_loadsol()
        self.left_force = raw_data[:,1:5]
        return self.left_force
    
    def get_right_force(self):
        """Returns an array containing the force data of the right insole ([Fheel, Fmedial, Flateral, Ftotal])."""
        raw_data = self.csv_reader_loadsol()
        self.right_force = raw_data[:,13:17]
        return self.right_force
    
    def get_left_accelero(self):
        """Returns an array containing the data of the accelerometer of the left insole ([AccX, AccY, AccZ])."""
        raw_data = self.csv_reader_loadsol()
        self.left_accelero = raw_data[:,6:9]
        return self.left_accelero

    def get_right_accelero(self):
        """Returns an array containing the data of the accelerometer of the right insole ([AccX, AccY, AccZ])."""
        raw_data = self.csv_reader_loadsol()
        self.right_accelero = raw_data[:,18:21]
        return self.right_accelero

    def get_left_gyro(self):
        """Returns an array containing the data of the gyrometer of the left insole ([GyroX, GyroY, GyroZ])."""
        raw_data = self.csv_reader_loadsol()
        self.left_gyro = raw_data[:,9:12]
        return self.left_gyro
    
    def get_right_gyro(self):
        """Returns an array containing the data of the gyrometer of the right insole ([GyroX, GyroY, GyroZ])."""
        raw_data = self.csv_reader_loadsol()
    ...
    def remove_duplicate_data(self):
        ...
        for i in index_duplicate:
            ...
            return time, left_force, right_force, left_accelero, right_accelero, left_gyro, right_gyro
    ...
    def suppress_outliers(self):
        ...
        index_outlier_left = [index for index, item in enumerate(left_force[:,0]) if item < 0]

        # Indexes of outliers of the right insole
        index_outlier_right = [index for index, item in enumerate(right_force[:,0]) if item < 0]
...
if __name__=="__main__":
    ...
    time, left_force_unique, right_force_unique, left_accelero_unique, right_accelero_unique, left_gyro_unique, right_gyro_unique = test.remove_duplicate_data()

    left_force, right_force = test.suppress_outliers()

    plt.plot(time,left_force)
    plt.show()
    plt.plot(time,right_force)
    plt.show()
