...
class DataLoadsol(Data):
    ...
    def suppress_incorrect_values(self):
        ...
        index_incorrect_values_left = [
            index for index, item in enumerate(self.raw_data_l_f_heel) if item < 0
        ]
        index_incorrect_values_right = [
            index for index, item in enumerate(self.raw_data_r_f_heel) if item < 0
        ]
    ...
    def suppress_duplicate_data(self):
        ...
        index_duplicate_left = [
            index
            for index, item in enumerate(self.raw_time_left)
            if item in self.raw_time_left[:index]
        ]

        index_duplicate_right = [
            index
            for index, item in enumerate(self.raw_time_right)
            if item in self.raw_time_right[:index]
        ]
    ...
    def fill_missing_data(self):
        ...
        max_nan_to_delete = 0
        for data_array in [
            self.filled_data_l_f_heel,
            self.filled_data_l_f_medial,
            self.filled_data_l_f_lateral,
            self.filled_data_l_f_total,
            self.filled_data_l_acc,
            self.filled_data_l_gyro,
            self.filled_data_r_f_heel,
            self.filled_data_r_f_medial,
            self.filled_data_r_f_lateral,
            self.filled_data_r_f_total,
            self.filled_data_r_acc,
            self.filled_data_r_gyro
        ]:
            for itr in range(1, len(data_array)):
                # if not np.isnan(data_array[-itr]) or not np.ma.all(data_array[-itr]):
        ...
        data_l_f_heel_filled = np.reshape(
            data_l_f_heel_filled.to_numpy(), (len(data_l_f_heel_filled),)
        )
        data_l_f_medial_filled = np.reshape(
            data_l_f_medial_filled.to_numpy(), (len(data_l_f_medial_filled),)
        )
        data_l_f_lateral_filled = np.reshape(
            data_l_f_lateral_filled.to_numpy(), (len(data_l_f_lateral_filled),)
        )
        data_l_f_total_filled = np.reshape(
            data_l_f_total_filled.to_numpy(), (len(data_l_f_total_filled),)
        )
        data_l_acc_filled = data_l_acc_filled.to_numpy()
        data_l_gyro_filled = data_l_gyro_filled.to_numpy()

        data_r_f_heel_filled = np.reshape(
            data_r_f_heel_filled.to_numpy(), (len(data_r_f_heel_filled),)
        )
        data_r_f_medial_filled = np.reshape(
            data_r_f_medial_filled.to_numpy(), (len(data_r_f_medial_filled),)
        )
        data_r_f_lateral_filled = np.reshape(
            data_r_f_lateral_filled.to_numpy(), (len(data_r_f_lateral_filled),)
        )
        data_r_f_total_filled = np.reshape(
            data_r_f_total_filled.to_numpy(), (len(data_r_f_total_filled),)
        )
    ...
    def get_filled_time(self):
        """Returns the filled time vector."""
...
if __name__ == "__main__":
    ...
    test = DataLoadsol(curr_path + "\\examples\\data\\test_pointe_1.txt", 200)
    ...
    plt.plot(test.raw_time_left, raw_data_l_f_tot, label="raw")
    test.suppress_incorrect_values()
    plt.plot(
        test.raw_time_left, test.raw_data_l_f_total, label="incorrect data suppressed"
    )
    plt.legend()
    plt.figure()
    ...
    filled_data_l_f_total = test.get_filled_data("LEFT", "F_TOTAL")

    plt.plot(test.filled_time, filled_data_l_f_total, label="interpolated")
    plt.plot(
        test.pre_processed_time,
        test.pre_processed_data_l_f_total,
        label="pre_processed",
    )
    plt.legend()
    plt.figure()
    resampled_data_l_f_total = test.downsample(
        100, insole_side="LEFT", data_type="F_TOTAL"
    )
    resampled_time = test.downsample(100, time=test.filled_time)

    filtered_data_l_f_total = test.filter_data(
        2, 20, 10, data=filled_data_l_f_total, column=0, dimension_1=True
    )

    plt.plot(
        test.filled_time,
        filled_data_l_f_total,
        "-o",
        label="filled",
        markersize=2,
    )
    plt.plot(
        resampled_time, resampled_data_l_f_total, "-o", label="resampled", markersize=2
    )
    plt.plot(
        test.raw_time_left, test.raw_data_l_f_total, "-o", label="raw", markersize=2
    )
    plt.plot(
        test.filled_time,
        filtered_data_l_f_total,
        "-o",
        label="filtered",
        markersize=2,
    )
