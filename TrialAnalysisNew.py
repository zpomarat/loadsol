...
class TrialAnalysis:
    def __init__(
        ...
        self.weight_normalized_data = None
        ...
        if data_state == "pre_processed":
            ...
            if DataForcePlates is not None:
                ...
                self.data_forceplates_sync = deepcopy(
                    self.data_forceplates.downsampled_data
                )
            ...
            if DataForcePlates is not None:
                ...
                self.data_forceplates_sync = deepcopy(
                    self.data_forceplates.filtered_data
                )
        ...
        if DataLoadSol is not None and DataForcePlates is not None:
            ...
            min_len = min(
                len(self.data_loadsol_sync["time"]),
                len(self.data_forceplates_sync["time"]),
            )
    ...
    def compare_loadsol_forceplates(self):
        ...
        axs[0].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_heel_l"],
            label="heel",
        )
        axs[0].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_medial_l"],
            label="medial",
        )
        axs[0].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_lateral_l"],
            label="lateral",
        )
        axs[0].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_total_l"],
            label="total",
        )
        axs[0].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fx1"],
            "-.",
            label="Fx",
        )
        axs[0].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fy1"],
            "-.",
            label="Fy",
        )
        axs[0].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fz1"],
            "-.",
            label="Fz",
        )
        ...
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_heel_r"],
            label="heel",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_medial_r"],
            label="medial",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_lateral_r"],
            label="lateral",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["f_total_r"],
            label="total",
        )
        axs[1].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fx2"],
            "-.",
            label="Fx",
        )
        axs[1].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fy2"],
            "-.",
            label="Fy",
        )
        axs[1].plot(
            self.data_forceplates_sync["time"],
            self.data_forceplates_sync["fz2"],
            "-.",
            label="Fz",
        )
    ...
    def compare_forces_imu(self):
        ...
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_x_l"],
            label="acc x",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_y_l"],
            label="acc y",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_z_l"],
            label="acc z",
        )
        ...
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_x_l"],
            label="gyro x",
        )
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_y_l"],
            label="gyro y",
        )
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_z_l"],
            label="gyro z",
        )
        ...
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_x_r"],
            label="acc x",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_y_r"],
            label="acc y",
        )
        axs[1].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["acc_z_r"],
            label="acc z",
        )
        ...
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_x_r"],
            label="gyro x",
        )
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_y_r"],
            label="gyro y",
        )
        axs[2].plot(
            self.data_loadsol_sync["time"],
            self.data_loadsol_sync["gyro_z_r"],
            label="gyro z",
        )
...
if __name__ == "__main__":
    ...
    for name, idx_ls, idx_fp in zip(trial_names, index_ls, index_fp):
        ...
        Trial = TrialAnalysis(
            DataLoadSol=data_ls,
            DataForcePlates=data_fp,
            final_frequency=200,
            sync_index_loadsol=idx_ls,
            sync_index_forceplates=int(idx_fp / 5),
            data_state="filtered",
        )

        mean_left = Trial.define_weight(side="left")
        mean_right = Trial.define_weight(side="right")

        Trial.export_csv(
            export_directory=curr_path + "\\tests_09_02_24\\results\\filtered\\",
            file_name=file_name,
        )
        ...
        Trial.plot(
            time=Trial.data_loadsol_sync["time"],
            data=Trial.data_loadsol_sync["f_total_l"],
            data_type="force",
            legend="f total left",
        )
        Trial.plot(
            time=Trial.data_forceplates_sync["time"],
            data=Trial.data_forceplates_sync["fy1"],
            data_type="force",
            legend="fy",
        )
