...
class TrialAnalysis:
        ...
        if self.DataForceplates is not None:
            if filtering is True:
                ...
                self.fp1_x = self.DataForceplates.sync_signals(
                    filtering=True,
                    start_sync=self.sync_time_forceplate,
                    forceplate_number=1,
                    column=0,
                )
    ...
    def correlation_ftot_acc(self, direction: str, start_trial: int, end_trial: int):
        match direction:
            case "x":
                corr_coef_l_ftot_accx = stats.pearsonr(
                    self.l_f_total[start_trial:end_trial],
                    self.l_accx[start_trial:end_trial],
                )
                print(
                    f"Pearson correlation coefficient between Ftot and Accx: {corr_coef_l_ftot_accx[0]}"
                )
            case "y":
                corr_coef_l_ftot_accy = stats.pearsonr(
                    self.l_f_total[start_trial:end_trial],
                    self.l_accx[start_trial:end_trial],
                )
                print(
                    f"Pearson correlation coefficient between Ftot and Accy: {corr_coef_l_ftot_accy[0]}"
                )
            case "z":
                corr_coef_l_ftot_accz = stats.pearsonr(
                    self.l_f_total[start_trial:end_trial],
                    self.l_accx[start_trial:end_trial],
                )
                print(
                    f"Pearson correlation coefficient between Ftot and Accz: {corr_coef_l_ftot_accz[0]}"
                )
