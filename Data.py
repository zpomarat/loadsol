...
class Data:
    ...
    def downsample(
        ...
        insole_side=None,
        data_type=None,
        forceplate_number=None,
        time=None,
    ):

        # Check if arguments are correctly entered
        if (
            insole_side is not None
            and forceplate_number is not None
            or data_type is not None
            and forceplate_number is not None
        ):
            raise AssertionError(
                "Cannot give both forceplate number & insole side / data type at the same time."
            )
        if (
            time is not None
            and forceplate_number is not None
        ...
        if self.frequency % final_frequency != 0:
            raise ValueError(
                "The final frequency is not a multiple of the original frequency."
            )
        ...
        if forceplate_number is not None:
            data = self.get_pre_processed_data(
                forceplate_number
            )  # Raw data from forceplate
        if time is not None:
    ...
    def filter_data(
        ...
        # Get the data
        if data_type is not None:
            data = self.downsample(
                insole_side=insole_side, data_type=data_type, final_frequency=200
            )  # Raw data from loadsol
        if forceplate_number is not None:
            data = self.downsample(
                forceplate_number=forceplate_number, final_frequency=200
            )  # Raw data from forceplate
        ...
        if data_type is not None:
            if filtering is True:
                data = self.filter_data(
                    ...
                    column=column,
                    insole_side=insole_side,
                    data_type=data_type,
                )
                return data[start_sync:]
            else:
                data = self.downsample(final_frequency=200,insole_side=insole_side,data_type=data_type)
                return data[start_sync:]
        if forceplate_number is not None:
            if filtering is True:
                data = self.filter_data(
                    ...
                    column=column,
                    dimension_1=dimension_1,
                    forceplate_number=forceplate_number,
                )
                return data[start_sync:]
            else:
                data = self.downsample(final_frequency=200,forceplate_number=forceplate_number)
                return data[start_sync:]
